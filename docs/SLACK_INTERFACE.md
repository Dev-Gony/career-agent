...
Slack 채널에서 사용자가 Career Agent를 호출하는 입력 경계를 정의한다. 실제 Slack 앱의 Token 인증, 개인 허용 목록 설정과 Socket Mode 연결을 완료했다. 합성 또는 실제 `app_mention` 이벤트를 검증하고, 정확 명령은 결정론적으로 처리하며 그 밖의 자연어는 Gemini planner가 제한된 내부 도구 계획으로 변환한다.

    Slack app_mention 예제
    -> 워크스페이스·앱·사용자·채널 검증
    -> 정확 명령 식별 또는 자연어 계획 생성
    -> 로컬 allowlist와 strict validator 확인
    -> 허용된 내부 도구 실행
    -> 같은 Slack 스레드에 결과 전달
...
상태 변경과 명시적 결정에는 다음 정확 입력을 유지한다.
...
첨부파일 1개와 봇 호출만 보내도 프로필 자료 입력으로 인식한다. 그 밖의 자연어는 Gemini planner가 다음 네 도구 중 필요한 작업만 선택한다.
...
planner 출력에는 자유문 응답이나 임의 도구 인자가 없다. 계획은 1~3단계, 도구 중복 금지이며 첨부 분석은 검증된 파일이 있을 때 첫 단계로만 허용한다. 공고 찾기 계획에는 사용자가 메시지에서 명시한 직무명만 최대 3개의 `search_focus_roles`로 포함할 수 있다. 이 값은 현재 실행의 검색 직무에만 쓰고 저장 요청과 영구 프로필에는 기록하지 않는다. URL·경로·Slack ID·Token·명령 구문·프롬프트 주입 문구는 로컬 validator가 전체 계획을 거부한다. 승인·거부, 외부 전송 동의, 경력·기술 매핑, 최종 프로필 반영, shell, 파일 경로와 URL은 모델 도구에 포함하지 않는다. 모델이 생성한 계획은 로컬 검증을 통과한 경우에만 기존 동작으로 매핑한다.

Slack 메시지 원문은 planner 호출 중 메모리에만 존재하고 `slack-command-request`에는 저장하지 않는다. Gemini 요청에는 정규화한 사용자 메시지, 검증된 첨부 존재 여부와 계약 버전만 포함하며 Slack 사용자·채널·스레드 ID, 파일명, 파일 ID, URL과 Token은 넣지 않는다. 동일 이벤트 요청은 planner 호출 전에 기존 저장 요청으로 중복 제거한다.
...
실제 Slack 공고 검색은 검증된 활성 개인 프로필을 우선 사용한다. 활성 개인 프로필이 없으면 동일 사용자·채널·문서에 연결된 명시적 승인이 있고 검증된 최신 외부 Gemini 초안으로 검색용 임시 프로필을 만든다. 초안의 직무 맥락과 기술은 미확정 검색 가설과 보조 신호로만 사용하고, 경력·성과는 확인된 적합 근거로 승격하지 않는다. 활성 프로필과 승인된 초안이 모두 없으면 공개 저장소의 예제 프로필로 조용히 대체하지 않고 검색을 중단해 이유를 안내한다. 임시 프로필은 subprocess 실행 동안만 전달하며, 결과에도 임시 근거 사용 사실을 표시한다.

매칭 규칙, 분석 파이프라인 또는 프로필이 바뀌었거나 기존 검토 큐에 현재 검색 계획 식별 정보가 없어 오래된 큐로 확인된 경우만 자동 복구한다. 고정된 큐 생성 스크립트로 새 큐를 만든 뒤 분석을 한 번만 재시도한다. 다른 오류는 자동 재시도하지 않고 고정 실패 안내를 반환한다.
