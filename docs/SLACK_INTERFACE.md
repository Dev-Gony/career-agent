...
성공 응답은 회사·공고명, 지원 판단, 우선 확인 항목, 학습·포트폴리오 과제 수와 남은 큐만 포함한다. 외부 공고에서 온 `<`, `>`, `&`는 Slack 제어 문자열로 해석되지 않도록 변환한다. 로컬 저장 경로, 원본 표준 출력과 오류 내용은 Slack에 보내지 않는다.
...
2026-09-15 실제 Slack 명령으로 검토 큐의 Moloco 공고 1건이 분석됐고, 분석 완료 수가 4건으로 늘고 분석 필요 수가 6건으로 줄었다. Slack SDK 응답 오류는 없었다.

다음 단계에서는 분석 결과의 사용자 검토 흐름을 연결한다.

1. 성공 요약에 검증된 공고 원문 링크를 포함한다.
2. 사용자가 적합·보류·부적합 판단을 Slack에서 명시적으로 입력할 수 있게 한다.
3. 기존 비공개 사용자 판단 기록과 후속 큐 병합에 연결한다.
