...
현재 연결 키는 허용 사용자가 본인 1명인 개인용 MVP라는 전제 아래 비공개 저장소의 최신 검증 추출 결과다. 여러 사용자를 허용하는 단계에서는 Slack 사용자 ID와 추출 ID를 연결하는 별도 비공개 인덱스로 교체해야 한다. 실제 사용자 문서에 대한 분석 초안 생성은 아직 외부 모델 전송 동의와 공급자 연결 전이므로 구현 범위 밖이다.
...
적용 결과는 경력 수행 근거 추가, 새 성과 객체 추가, 기존 기술 근거 추가와 새 기술 추가를 지원한다. 적용 이력에는 프로필 내용을 복제하지 않고 변경 ID, 대상 ID와 동작만 기록한다. 새 프로필 버전은 별도 디렉터리에 저장하며 기준 프로필 파일을 덮어쓰지 않는다. 생성된 새 버전을 공고 검색의 활성 프로필로 전환하는 절차는 다음 단계다.
...
궁극적인 Slack Agent 흐름을 완성하기 위한 다음 우선순위는 추출 후보의 사용자 확인과 프로필 적용이다.

1. 사용자가 이력서, 포트폴리오, 경력기술서 또는 직무분석표 한 개를 첨부한다.
2. 파일을 비공개 원본 저장소에 가져오고 저장 직후 검토용 프로필 후보를 만든다.
3. Slack에서 후보 요약과 항목 한 건을 보여주고 사용자가 확인한 뒤 새 개인 프로필과 검색 계획을 생성한다.
