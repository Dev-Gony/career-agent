...
실제 Slack 공고 검색은 검증된 활성 개인 프로필을 필수로 한다. 활성 개인 프로필이 없으면 공개 저장소의 예제 프로필로 조용히 대체하지 않고 검색을 중단해 이유를 안내한다. 사용자 확인 전 이력서 분석 초안을 검색 보조 근거로 사용하는 별도 경로는 영구 프로필을 바꾸지 않는 임시 파일을 subprocess 실행 동안만 전달하며, 결과에도 임시 근거 사용 사실을 표시한다.
