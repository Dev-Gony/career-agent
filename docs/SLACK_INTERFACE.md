...
현재 연결 키는 허용 사용자가 본인 1명인 개인용 MVP라는 전제 아래 비공개 저장소의 최신 검증 추출 결과다. 여러 사용자를 허용하는 단계에서는 Slack 사용자 ID와 추출 ID를 연결하는 별도 비공개 인덱스로 교체해야 한다. 실제 사용자 문서의 외부 전송 동의 기록은 구현했지만, 동의된 내용을 공급자에 보내 분석 초안을 생성하는 단계는 아직 연결하지 않았다.
...
TXT·Markdown·DOCX에서 후보 문장이 한 개 이상 만들어지면 같은 Slack 스레드에 외부 전송 범위를 안내하고 `외부 AI 분석 동의` 또는 `외부 AI 분석 거부` 답변을 요청한다. 후보에는 경력, 회사와 프로젝트 정보가 포함될 수 있음을 알리고 동의 전에는 외부 전송을 실행하지 않는다.

동의 세션은 워크스페이스, 채널, 사용자, 스레드, 추출 결과 ID, 최소 공급자 요청의 SHA-256 지문, 공급자와 모델에 묶인다. 결정 기록도 정확한 세션·사용자·스레드를 포함하며 후보 문장, 파일 내용과 메시지 원문을 복제하지 않는다. 다른 사용자·채널·스레드의 답변과 스레드 밖 답변은 연결하지 않고, 같은 스레드에서는 가장 최신 문서 요청만 결정 대상으로 사용한다. 거부 시 외부 전송과 프로필 변경을 모두 실행하지 않는다. 현재 단계는 결정 기록까지만 구현했으며 승인이 저장돼도 외부 호출은 실행하지 않는다.

2026-09-22 확인한 Gemini API 공식 약관에 따라 무료 개발 키에는 실제 사용자 문서를 보내지 않는다. 무료 등급은 입력과 응답이 제품·모델 개선과 사람 검토에 사용될 수 있으므로 합성 fixture만 허용한다. 실제 문서 전송은 유료 등급의 데이터 처리와 보존 조건을 다시 검토한 뒤 별도 구현하며, 동의 기록만으로 이 제한을 해제하지 않는다.
...
궁극적인 Slack Agent 흐름을 완성하기 위한 다음 우선순위는 동의된 실제 문서의 검증 초안 생성이다.

1. 사용자가 이력서, 포트폴리오, 경력기술서 또는 직무분석표 한 개를 첨부한다.
2. 파일을 비공개 원본 저장소에 가져오고 저장 직후 검토용 프로필 후보를 만든다.
3. 외부 AI 전송 범위를 보여주고 문서별 명시적 동의를 받은 최소 텍스트만 분석해 검증 초안을 만든다.
4. Slack에서 후보 요약과 항목 한 건을 보여주고 사용자가 확인한 뒤 새 개인 프로필을 활성화한다.
