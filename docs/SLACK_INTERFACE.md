...
외부 AI 분석 동의·거부 기록 경계는 구현되어 있지만 현재 Slack 수신기는 동의를 자동 요청하지 않는다. 실제 TXT·Markdown·DOCX는 외부 전송 없는 로컬 초안을 기본 경로로 사용한다. 허용된 유료 공급자와 데이터 처리 조건을 확정한 뒤에만 외부 전송 범위와 경력·회사·프로젝트 정보 포함 가능성을 안내하고 해당 문서 스레드에서 동의 또는 거부를 받는다.

기능을 활성화할 경우 동의 세션은 워크스페이스, 채널, 사용자, 스레드, 추출 결과 ID, 최소 공급자 요청의 SHA-256 지문, 공급자와 모델에 묶인다. 결정 기록도 정확한 세션·사용자·스레드를 포함하며 후보 문장, 파일 내용과 메시지 원문을 복제하지 않는다. 다른 사용자·채널·스레드의 답변과 스레드 밖 답변은 연결하지 않고, 같은 스레드에서는 가장 최신 문서 요청만 결정 대상으로 사용한다. 현재 저장된 승인 기록만으로 외부 호출은 실행되지 않는다.
...
3. 외부 전송 없는 로컬 규칙으로 원문 근거형 검증 초안을 자동 생성한다.
4. Slack에서 후보 요약과 항목 한 건을 보여주고 사용자가 확인한 뒤 새 개인 프로필을 활성화한다.
5. 활성 프로필에서 검색 계획을 다시 만들고 공고 발견·분석과 사용자 피드백 기록으로 이어간다. 외부 AI 분석은 유료 공급자의 데이터 처리 조건을 별도로 검토한 경우에만 문서별 동의 지문을 다시 확인해 추가한다.
