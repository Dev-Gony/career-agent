...
이 단계는 읽기 전용이다. 항목을 표시해도 승인·거부 기록, 개인 프로필과 공고 검색 조건은 변경하지 않는다. 실제 사용자 문서의 분석 초안이 없으면 문단 분류까지만 완료됐다고 안내한다. 다음 단계에서 같은 스레드의 자연어 답변을 표시된 항목과 안전하게 연결한다.
