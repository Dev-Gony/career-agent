...
planner 출력에는 도구 인자와 자유문 응답이 없다. 계획은 1~3단계, 도구 중복 금지이며 첨부 분석은 검증된 파일이 있을 때 첫 단계로만 허용한다. 승인·거부, 외부 전송 동의, 경력·기술 매핑, 최종 프로필 반영, shell, 파일 경로와 URL은 모델 도구에 포함하지 않는다. 모델이 생성한 계획은 로컬 검증을 통과한 경우에만 기존 동작으로 매핑한다.
