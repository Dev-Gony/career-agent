...
1. `high`, `medium`과 직무 근거가 있는 `review` 후보만 사용하고 `low`는 제외한다.
...
6. 완전 일치 표현이 없는 `review` 후보는 Agent 생성 검색 계획의 직무 표현을 목표 직무 축별로 다시 비교한다.
7. `review` 후보는 같은 직무 축의 단어가 두 개 이상 일치하거나 `ITSM`, `FinOps`처럼 한 단어 자체가 고유한 검색 표현이어야 한다.
8. `Engineer`, `엔지니어`는 다른 구별 신호와 함께 있을 때만 인정하며 영문·한글 번역 중복을 서로 다른 근거로 세지 않는다.
9. 나머지 순서는 실제 시간대로 해석한 공고 갱신 시각의 최신순이다.
10. 기업 보드와 외부 공고 ID가 같은 항목은 하나만 유지한다.
11. 기본 큐 크기는 최대 10건이며 후보가 적으면 실제 후보 수만 저장한다. 설정 가능한 최대치는 50건이다.

`review`를 포함하는 이유는 완전 일치 기반 발견 규칙이 놓친 관련 직무를 찾아 false negative, 즉 관련 공고 누락을 검토하기 위해서다. 일반적인 직무 단어 하나만 일치하는 후보까지 상세 조회하지는 않는다.
