...
- 프로필에서 실제 부족으로 확인된 `skill` 또는 `cloud` 판정만 사용한다.
- 정보가 없는 `unknown`과 추가 확인이 필요한 `partial`은 학습 과제로 바꾸지 않는다.
- 필수 조건을 우대 조건보다 먼저 처리하고 한 분석에서 최대 1개만 추천한다.
- 완료된 기존 프로젝트가 있으면 새 프로젝트보다 기존 프로젝트 확장을 우선한다.
- 우대 조건에만 있는 클라우드 부족은 실제 공고 여러 건의 반복 수요를 집계하기 전까지 보류한다.
- 완료 여부는 강의 수강이 아니라 코드·설정, 실행 문서와 재현 성공 로그로 확인한다.
...
- 한 분석에서 최대 1개만 추천한다.
- 학습 추천이 기존 완료 프로젝트를 대상으로 하면 같은 행동을 포트폴리오 변경점과 연결한다.
- 학습 추천이 없으면 `match` 또는 `partial` 판정 중 완료 프로젝트의 직접 증거와 구체적인 다음 행동이 함께 있는 주요 업무만 사용한다.
- `unknown` 업무는 경험 부재로 단정하지 않고 포트폴리오 추천에서도 제외한다.
- 계획 중인 프로젝트는 이미 증명된 기반이 아니므로 첫 개선 대상으로 선택하지 않는다.
- 기대 증거에는 변경 코드, 실행 방법과 개선 기능의 동작 로그를 포함한다.
