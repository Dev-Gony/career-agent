...
- 실제 외부 LLM 호출: OpenAI 어댑터, 공개 합성 Gemini 경로와 명시적 승인 기반 실제 문서 Gemini 경로 구현
- 실제 문서 외부 전송 동의: Slack 문서 스레드와 최소 요청 지문에 묶인 승인·거부 기록 구현
- 실제 문서 로컬 분석: TXT·Markdown·DOCX 추출 직후 근거형 검토 초안 자동 생성 구현
- Slack 표시: 최신 검증 초안의 항목 수 요약과 항목별 표시 구현
- 항목 승인·거부: 같은 사용자·채널·스레드의 자연어 답변 연결 구현
- 프로필 변경 제안: 최신 승인 항목의 비파괴 매핑 제안 구현

현재 구현은 실제 문서 후보를 로컬 규칙으로 즉시 초안화하고, 사용자가 문서별 Slack 세션에서 명시적으로 승인한 경우에는 같은 최소 후보를 Gemini로 추가 분석할 수 있다. 외부 호출 전 현재 문서 지문, 공급자·모델, 최신 세션과 사용자·채널·스레드 결합을 다시 검증한다. 모든 Gemini 근거 문자열은 참조 후보의 연속된 원문 구간 검증을 통과해야 하며 기술 숙련도는 항상 미확정이다. 검증 실패 응답은 저장하지 않는다.
...
1. 실제 Slack 첨부에서 로컬 초안, 항목 검토, 최종 프로필 승인까지 전체 흐름을 확인한다.
2. 유료 공급자를 선택할 때 데이터 처리·보존 조건을 다시 검토하고 기존 문서별 동의 지문을 호출 직전에 검증한다.
3. 활성 프로필에서 검색 계획을 다시 생성하고 공고 발견·분석 전체 흐름을 검증한다.
