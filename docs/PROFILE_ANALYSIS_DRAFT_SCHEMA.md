...
1. 변경 제안의 매핑 대기 항목을 Slack에서 한 건씩 확인한다.
2. 경력 대상 선택과 새 기술 숙련도 확인을 안전한 답변 형식으로 연결한다.
3. 모든 매핑이 확인된 변경 제안만 별도의 최종 승인 대상으로 표시한다.
4. 최종 승인 뒤에만 새 개인 프로필 버전을 만들고 검색 계획을 다시 생성한다.
