...
현재 구현은 실제 문서 후보를 보수적인 로컬 규칙으로 분석해 검토 초안을 자동 생성한다. 기간과 행동이 함께 있는 경력, 수치와 결과 표현이 함께 있는 성과, 기술명과 사용 행동이 함께 있는 기술만 근거 후보로 만든다. 부정, 미경험, 학습 중과 희망 표현은 긍정 근거로 승격하지 않으며 기술 숙련도는 항상 미확정이다. 모든 근거 문자열은 원문 구간 검증을 다시 통과하고 외부 네트워크 요청은 발생하지 않는다. 합성 공급자와 외부 전송 동의 경계도 별도로 유지하며, 동의된 실제 이력서 후보를 외부 API에 보내는 단계는 연결하지 않았다.
