...
검토 큐는 `private-data/review-queues/`에 새 파일로 저장됩니다. `high`, `medium` 다음에 서울 지역과 프로필 생성 직무 표현이 가까운 `review` 후보를 배치하여, 기존 완전 일치 규칙이 놓친 관련 공고도 검증 표본에 포함합니다. 과거 발견 기록에 `review`로 남은 채용관심등록·인재풀 제목도 큐 생성 시 다시 확인해 제외합니다. 큐 생성은 추가 상세 조회를 하지 않으며 각 후보가 최신 규칙으로 이미 분석됐는지만 표시합니다.
