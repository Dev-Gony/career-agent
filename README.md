...
2. Slack에서 분석 초안 항목 확인과 승인·거부
3. 승인 결과를 새 개인 프로필과 검색 계획으로 연결
4. 갱신된 검색 계획으로 공고 발견·분석을 다시 실행
...
`@career_break 프로필 검토 시작`을 보내면 최신 검증 초안의 경력·성과·기술·추가 질문 중 첫 항목 한 건을 같은 스레드에 표시합니다. Slack 제어 문자열을 무해화하고 후보 ID, 초안 ID, 공급자와 로컬 경로는 숨깁니다. 현재는 읽기 전용이므로 이 명령만으로 승인·거부를 기록하거나 개인 프로필을 변경하지 않습니다. 실제 사용자 문서에 연결된 분석 초안이 없으면 아직 분석 초안이 없다고 안내합니다.
