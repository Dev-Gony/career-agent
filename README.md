...
1. 자동 생성 검색 계획의 직무 확장어와 프로필·계획 일치 검증을 보강
...
현재 구현은 공식 인크루트 RSS와 설정에 등록된 Greenhouse 기업 보드 목록을 읽고 로컬 JSON에 신규 후보를 중복 없이 저장합니다. Greenhouse 후보는 활성 프로필에서 자동 생성한 목표 직무와 선호 조건으로 정렬하며, 여러 보드 전체의 현재 후보 중 한 번에 1건을 공고 ID 입력이나 복사·붙여넣기 없이 상세 조회·분석·저장합니다. 조건에 맞는 새 후보가 없으면 무관한 공고를 분석하지 않고 정상 빈 결과를 반환합니다. 각 실행의 상태와 분석 참조도 별도 보존합니다. 확인된 기술 부족이 있으면 기존 완료 프로젝트를 활용하는 가장 작은 학습 및 포트폴리오 개선 과제 1개를 생성합니다. 실제 검증을 위해 현재 후보의 메타데이터 검토 큐를 최대 10건까지 만들고 사용자의 실제 판단을 별도 비공개 기록으로 남길 수 있습니다. 사용자 문서 원본을 검증해 비공개 저장하고 UTF-8 텍스트·Markdown·DOCX에서 검토용 프로필 후보와 로컬 검증 초안을 만든 뒤 승인 기반의 새 프로필 버전으로 적용할 수 있습니다. 실제 Slack 수신과 스레드 답변, 원문 링크와 판단 근거를 포함한 분석 요약, Slack 첨부파일의 인증 다운로드·비공개 저장과 저장 직후 로컬 후보 추출을 확인했습니다. 로컬 규칙과 합성 LLM 응답은 같은 strict JSON 계약으로 검증하며 존재하지 않는 후보 참조, 원문에 없는 근거 문구와 확정되지 않은 기술 숙련도를 거부합니다. 외부 공급자는 명시적 승인 없이 호출하지 않고 무료 Gemini에는 실제 문서를 보내지 않습니다. 저장된 초안은 ID와 내용 지문을 다시 검증하고 같은 추출 결과의 최신 검토 대기 초안만 선택할 수 있습니다. Slack에서는 검토 항목을 한 건씩 표시하고 같은 사용자·채널·스레드의 `맞아`와 `제외해줘`만 승인·거부 기록으로 연결하며 이미 검토한 항목을 건너뜁니다. 최신 승인 항목은 경력·성과·기술별 비파괴 변경 제안으로 변환하며 경력 대상과 새 기술 숙련도를 임의로 추정하지 않고 같은 Slack 스레드에서 허용값으로 확인해 별도 기록합니다. 모든 선택을 마친 변경은 적용 전 최종 변경안으로 묶어 Slack에서 전체 요약을 확인하고, 같은 스레드의 명시적 최종 승인 뒤에만 원본과 분리된 새 비공개 프로필 버전으로 적용합니다. 승인된 버전은 다음 공식 공고 발견, 검토 큐와 상세 분석의 활성 프로필로 사용되고 검색 계획도 같은 프로필에서 자동 재생성됩니다. 현재 궁극적인 개인용 MVP 진행률은 약 92%입니다. 아직 자동 검색 계획의 후보 재현율 보강, 실제 Slack 전체 재검증, 사용자 피드백 대화, PDF 본문 추출과 채용 소스 확대가 남아 있습니다.
