...
현재는 실제 Slack Socket Mode 연결에서 첫 `app_mention`을 수신·검증하고 고정 확인 응답을 보내는 단계입니다.
...
3. 실제 Slack `app_mention` 수신·응답을 확인한 뒤 다음 공고 분석 동작 연결
4. 하루 1회 예약 실행
...
실행 중 지정 채널에서 `@career_break 다음 공고 찾아줘`를 보내면, 허용된 사용자와 채널의 이벤트만 저장하고 현재 단계에서는 공고 분석을 실행하지 않았다는 고정 문구로 답합니다. 답변은 원래 메시지의 스레드에 한 번만 보내며 같은 `event_id` 재전송에는 중복 답변하지 않습니다. 종료는 `Ctrl+C`입니다.
