...
모든 분석 항목 검토를 마친 뒤 `@career_break 프로필 변경 검토 시작`을 보내면 최신 결정을 사용해 비파괴 변경 제안을 만들고 첫 매핑 대기 항목을 표시합니다. 경력·성과에는 연결 가능한 기존 경력 ID를 보여주고, 새 기술에는 허용된 숙련도 값을 보여줍니다. 현재 단계에서는 답변 형식만 안내하며 경력 선택과 숙련도 결정을 저장하거나 실제 프로필을 변경하지 않습니다.
