...
외부 AI 분석 동의·거부 기록 경계는 구현되어 있지만 현재 Slack 수신기는 이 동의를 자동 요청하지 않습니다. 실제 문서는 외부 전송 없는 로컬 초안으로 처리되며, 허용된 유료 공급자와 데이터 처리 조건을 확정하기 전에는 외부 분석 기능을 사용자에게 제시하지 않습니다. 향후 기능을 활성화할 때는 추출 결과 ID, 최소 전송 요청의 SHA-256 지문, 공급자·모델, 정확한 Slack 세션·사용자·스레드와 결정만 저장하고 후보 문장과 파일 내용은 동의 기록에 복제하지 않습니다. 무료 Gemini 개발 키에는 실제 문서 내용을 보내지 않습니다.
