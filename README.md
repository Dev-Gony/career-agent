...
1. 주요 업무와 사용자 프로젝트·경력 증거 연결
2. 강점, 부족, 미확인 항목을 전체 결과 구조로 생성
...
통합 명령은 아직 평가하지 못하는 조건도 결과에서 누락하지 않고 `unknown`으로 표시합니다. 필수·우대 조건, 경력·학력·지역·고용 형태의 지원 가능 조건과 근거 기반 지원 추천을 출력합니다. 추천은 합격 확률이 아니라 현재 프로필과 공고의 비교 결과입니다.
...
현재 구현은 공식 인크루트 RSS를 읽고 로컬 JSON에 신규 후보를 저장하며, 구조화된 예제 공고의 기술·경험 요구사항과 지원 가능 조건을 비교해 지원 판단과 근거를 생성합니다. 아직 LLM 호출, 상세 공고 수집, 주요 업무 매칭 또는 Slack 연동은 하지 않습니다.
...
현재 상태: 자동 공고 발견 및 근거 기반 지원 추천 구현

공식 인크루트 RSS를 읽어 프로필 기반 발견 레코드로 변환하고 실행 간 중복을 제거해 로컬에 저장하는 첫 동작 가능한 기능을 구현했습니다. 실제 첫 실행은 신규 20건, 두 번째 실행은 신규 0건과 중복 20건으로 확인했습니다. 구조화된 예제 공고에서는 필수·우대·지원 가능 조건을 분리해 비교하고 `적극 지원` 등 다섯 가지 지원 판단 중 하나를 근거와 함께 생성할 수 있습니다. 다음에는 공고의 주요 업무를 사용자 프로젝트·경력 증거에 연결합니다.
