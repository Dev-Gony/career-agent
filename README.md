...
1. 실제 사용자 문서를 외부 AI 분석 공급자로 보내기 전 Slack에서 문서별 명시적 동의를 기록
2. 동의된 최소 텍스트만 Gemini 개발용 공급자로 분석하고 검증된 초안을 생성
3. 활성 프로필에서 검색 계획을 다시 생성하고 공고 발견·분석 전체 흐름을 재실행
4. 실제 Slack 첨부부터 개인화 공고 추천까지 전체 흐름을 검증
5. 추천에 대한 사용자 피드백과 공식 채용 소스를 단계적으로 확장
