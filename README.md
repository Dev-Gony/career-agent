...
1. 사용자 동의가 있는 실제 문서 분석 공급자 연결과 검증 초안 생성
2. 변경 제안의 경력 대상과 새 기술 숙련도 답변을 안전한 결정 기록으로 저장
3. 별도 최종 승인 뒤 새 개인 프로필 버전으로 적용
4. 승인 결과를 새 개인 프로필과 검색 계획으로 연결
5. 갱신된 검색 계획으로 공고 발견·분석을 다시 실행
...
모든 매핑 선택을 마친 뒤 `@career_break 프로필 최종 검토`를 보내면 경력 수행 근거, 성과 근거, 기존 기술 근거와 새 기술 추가를 하나의 불변 최종 변경안으로 합쳐 같은 스레드에 표시합니다. 기준 프로필이 달라졌거나 선택이 빠졌으면 최종 변경안을 만들지 않습니다. 내부 제안·검토 ID는 공개하지 않고 실제 적용 전 변경 개수, 대상 경력·기술과 근거 요약만 보여줍니다. 현재 단계에서는 최종 변경안도 개인 프로필에 적용하지 않습니다.
