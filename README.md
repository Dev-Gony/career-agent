...
현재 구현은 공식 인크루트 RSS와 설정에 등록된 Greenhouse 기업 보드 목록을 읽고 로컬 JSON에 신규 후보를 중복 없이 저장합니다. Greenhouse 후보는 프로필에서 도출한 목표 직무와 선호 조건으로 정렬하며, 여러 보드 전체의 현재 후보 중 한 번에 1건을 공고 ID 입력이나 복사·붙여넣기 없이 상세 조회·분석·저장합니다. 조건에 맞는 새 후보가 없으면 무관한 공고를 분석하지 않고 정상 빈 결과를 반환합니다. 각 실행의 상태와 분석 참조도 별도 보존합니다. 확인된 기술 부족이 있으면 기존 완료 프로젝트를 활용하는 가장 작은 학습 및 포트폴리오 개선 과제 1개를 생성합니다. 실제 검증을 위해 현재 후보의 메타데이터 검토 큐를 최대 10건까지 만들고 사용자의 실제 판단을 별도 비공개 기록으로 남길 수 있습니다. 사용자 문서 원본을 검증해 비공개 저장하고 UTF-8 텍스트에서 검토용 프로필 후보를 만든 뒤 승인 기반의 새 프로필 버전으로 적용할 수 있습니다. 실제 Slack 수신과 스레드 답변, 원문 링크와 판단 근거를 포함한 분석 요약, Slack DOCX의 인증 다운로드와 비공개 저장을 확인했습니다. 현재 궁극적인 개인용 MVP 진행률은 약 70%입니다. 아직 Slack 저장 문서와 프로필 추출 파이프라인 연결, 실제 개인 프로필 기반 검색 계획 갱신, 사용자 피드백 대화, PDF·DOCX 본문 추출, 채용 소스 확대 또는 LLM 호출은 구현하지 않았습니다.
