...
현재는 실제 Slack 명령으로 다음 공고 1건 분석과 스레드 요약 응답을 검증했고, 저장된 최신 프로필 분석 초안을 원문 없이 조회하는 명령을 합성 이벤트로 검증한 단계입니다.
...
1. 무료 Gemini에는 합성 문서만 보내 외부 공급자 연결을 검증하고 실제 문서는 로컬 분석 경로를 먼저 완성
2. 실제 Slack 첨부부터 개인화 공고 추천까지 전체 흐름을 검증
3. 추천에 대한 사용자 피드백과 공식 채용 소스를 단계적으로 확장
...
공고 발견·검토 큐·상세 분석 스크립트는 명시적 `--search-plan` 입력이 없으면 선택된 활성 프로필에서 검색 계획을 즉시 다시 만듭니다. 목표 직무 원문, 실제 근거가 있는 `basic`·`project`·`work` 기술, 프로필에 명시된 지역·고용 형태와 업무 선호만 사용합니다. `exposure`·`learning` 기술, 경력 연수와 재택 선호는 검색 신호로 추정하지 않습니다. 낮은 선호는 강제 제외가 아니라 순위 하향에만 사용합니다.
...
후보가 만들어지면 같은 Slack 스레드에서 외부 AI 분석 범위와 개인 경력 정보 포함 가능성을 먼저 안내합니다. `@career_break 외부 AI 분석 동의` 또는 `@career_break 외부 AI 분석 거부`만 해당 사용자·채널·스레드의 결정으로 연결합니다. 기록에는 추출 결과 ID, 최소 전송 요청의 SHA-256 지문, 공급자·모델, 정확한 Slack 세션·사용자·스레드와 결정만 저장하며 후보 문장과 파일 내용은 복제하지 않습니다. 한 스레드에서는 가장 최신 문서 요청만 결정 대상으로 사용합니다. 현재 단계는 동의 기록까지만 구현했으며 승인만으로 외부 전송을 실행하지 않습니다. 무료 Gemini 개발 키에는 실제 문서 내용을 보내지 않습니다.
...
2026-09-22에 확인한 Gemini API 공식 약관상 무료 등급 입력과 응답은 Google 제품·모델 개선과 사람 검토에 사용될 수 있으므로, 이름과 연락처를 제거한 실제 경력 문장도 보내지 않습니다. 실제 문서의 외부 분석은 유료 등급의 데이터 처리 조건과 보존 기간을 다시 검토한 뒤 별도로 허용합니다. 유료 등급도 오용 감시를 위한 보존이 있을 수 있으므로 완전한 무보존으로 가정하지 않습니다. 근거는 [Gemini API 추가 이용약관](https://ai.google.dev/gemini-api/terms), [오용 감시 정책](https://ai.google.dev/gemini-api/docs/usage-policies), [Zero data retention 안내](https://ai.google.dev/gemini-api/docs/zdr)다.
...
현재 구현은 공식 인크루트 RSS와 설정에 등록된 Greenhouse 기업 보드 목록을 읽고 로컬 JSON에 신규 후보를 중복 없이 저장합니다. Greenhouse 후보는 활성 프로필에서 자동 생성한 목표 직무와 선호 조건으로 정렬하며, 여러 보드 전체의 현재 후보 중 한 번에 1건을 공고 ID 입력이나 복사·붙여넣기 없이 상세 조회·분석·저장합니다. 조건에 맞는 새 후보가 없으면 무관한 공고를 분석하지 않고 정상 빈 결과를 반환합니다. 각 실행의 상태와 분석 참조도 별도 보존합니다. 확인된 기술 부족이 있으면 기존 완료 프로젝트를 활용하는 가장 작은 학습 및 포트폴리오 개선 과제 1개를 생성합니다. 실제 검증을 위해 현재 후보의 메타데이터 검토 큐를 최대 10건까지 만들고 사용자의 실제 판단을 별도 비공개 기록으로 남길 수 있습니다. 사용자 문서 원본을 검증해 비공개 저장하고 UTF-8 텍스트·Markdown·DOCX에서 검토용 프로필 후보를 만든 뒤 승인 기반의 새 프로필 버전으로 적용할 수 있습니다. 실제 Slack 수신과 스레드 답변, 원문 링크와 판단 근거를 포함한 분석 요약, Slack 첨부파일의 인증 다운로드·비공개 저장과 저장 직후 로컬 후보 추출을 확인했습니다. 합성 LLM 응답은 strict JSON 계약으로 검증하며 존재하지 않는 후보 참조, 원문에 없는 근거 문구와 확정되지 않은 기술 숙련도를 거부하는 비공개 프로필 분석 초안으로 저장할 수 있습니다. 공급자 중립 인터페이스는 로컬 합성 공급자 전체 흐름을 지원하며 외부 공급자를 명시적 승인 없이 호출하지 않습니다. 실제 첨부 문서의 외부 분석은 같은 Slack 스레드에서 전송 범위를 안내하고 문서 요청 지문에 묶인 명시적 동의·거부 기록을 저장합니다. 저장된 초안은 ID와 내용 지문을 다시 검증하고 같은 추출 결과의 최신 검토 대기 초안만 선택할 수 있습니다. Slack에서는 검토 항목을 한 건씩 표시하고 같은 사용자·채널·스레드의 `맞아`와 `제외해줘`만 승인·거부 기록으로 연결하며 이미 검토한 항목을 건너뜁니다. 최신 승인 항목은 경력·성과·기술별 비파괴 변경 제안으로 변환하며 경력 대상과 새 기술 숙련도를 임의로 추정하지 않고 같은 Slack 스레드에서 허용값으로 확인해 별도 기록합니다. 모든 선택을 마친 변경은 적용 전 최종 변경안으로 묶어 Slack에서 전체 요약을 확인하고, 같은 스레드의 명시적 최종 승인 뒤에만 원본과 분리된 새 비공개 프로필 버전으로 적용합니다. 승인된 버전은 다음 공식 공고 발견, 검토 큐와 상세 분석의 활성 프로필로 사용되고 검색 계획도 같은 프로필에서 자동 재생성됩니다. 현재 궁극적인 개인용 MVP 진행률은 약 89%입니다. 아직 실제 사용자 문서의 검증 초안 자동 생성, 사용자 피드백 대화, PDF 본문 추출과 채용 소스 확대는 구현하지 않았습니다.
